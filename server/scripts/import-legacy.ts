...
// cp1252 differs from latin1 only in the 0x80–0x9F range. phpMyAdmin's broken
// export path writes UTF-8 bytes, views them as cp1252, and re-encodes. To
// reverse it, we need to map those "fancy" codepoints back to their 0x80-range
// byte values before doing the utf-8 round-trip.
...
/**
 * Undo mojibake from phpMyAdmin exports. Works for Arabic, Cyrillic, CJK, and
 * Latin-diacritic text that was stored as UTF-8 but exported through a cp1252
 * lens. ASCII-only strings pass through untouched.
 */
